...
F = load('meshAA_.B_ep0.5_100100_sym.mat');
...
P = mu*B + pi*1i*fliplr(zup);

[C, h] = contourf(real(fdown), imag(fdown), abs(P + 1i*pi*zdown), 50);
...
figure
[C, h] = contourf(real(fdown), imag(fdown), abs(zdown), 50);
hold on
[C, h] = contourf(real(fup), imag(fup), abs(zup), 50);
