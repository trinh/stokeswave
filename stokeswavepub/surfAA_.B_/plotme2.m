function plotme
    F = load('meshAA_ep0.5_100200.mat');
...
%     brighten(0.5);
    ...
    handles = qt_bdint_path_plotter(path, pathdat, 'PatchWidth', 0.07, 'NumArc', 250, 'ZPlot', 'imag', 'ZFunc', 'dfdz');
...
%     delete(handles.surf.edge_left{1})
    ...
    function plotsurf_up
        ...
        [C, h] = contour3(X, Y, Z, [-10:0.3:10]);
        delete(h)
        S = contourdata(C);

        for j = 1:length(S)
            plot3(S(j).x, S(j).y, minz + 0*S(j).x, 'Color', contourcol);
        end
    end
    function plotsurf_down
        X = real(meshpathdat.down.FF);
        Y = imag(meshpathdat.down.FF);
        Z = reim(exp(meshpathdat.down.taumat - 1i*meshpathdat.down.thetamat));
        
        [C, h] = contour3(X, Y, 0.13+ Z, [-10:0.3:10]);
        delete(h)
        S = contourdata(C);
