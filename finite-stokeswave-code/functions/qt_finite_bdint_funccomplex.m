function [F, theta] = qt_finite_bdint_funccomplex(w, Y, wave, number_of_pass, a)
...
%       
%       NB: For finite-depth implementation, the only difference is the
%       addition of the I-Integral, so we can define
%           Hnew = Hilbert + I-Integral
%
%   2026-05-20: Update to clean up and get working
...
Iint = @(phi, f) qt_finite_bdint_Iint_from_series(phi, f, wave.an, wave.r0, wave.N);

% Calculate the Hilbert + I-Integral
for k = 1:length(w)
    H(k) = qt_finite_getint_gauss(w(k), wave);
    if wave.r0 == 0
        I(k) = 0;
    else
