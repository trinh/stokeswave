...
%
% Note: 
%           delta + i*beta = dz/df (JMVB Book eqn (6.3))
...
% The first (N-1) equations are given by an integral relationship of 
% delta - integral = 0 (see JMVB Book (6.20))
for k=1:N-1
    ...
    % Manual trapezoid rule for the integral 
    g = 2*y;
    g(1) = g(1)/2;
    g(N) = g(N)/2;
    integral = h/2*sum(g);    
