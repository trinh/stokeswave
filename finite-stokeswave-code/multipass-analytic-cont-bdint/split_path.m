...
for j=1:100000 %ensures loop is longer than as yet unknown size of A
        if j<= length(A)-1
            if (imag(A(j))< 0) && (imag(A(j+1))> 0)
                ratio=(0-imag(A(j)))/(imag(A(j+1))-imag(A(j)));
                
                a = real(A(j))+ratio*(real(A(j+1))-real(A(j))); 
                
                   BEFORE = A(1:j);
                   AFTER = A(j+1:end);
                   A=[BEFORE a+0*1i-pm NaN*(1+1i) a+0*1i+pm AFTER];
                  
a = (0-imag(A(j)))/(imag(A(j+1))-imag(A(j)))*(real(A(j+1))-real(A(j)));

            elseif (imag(A(j))> 0) && (imag(A(j+1))< 0)
                    ratio=(0-imag(A(j)))/(imag(A(j+1))-imag(A(j)));
                    a = real(A(j))+ratio*(real(A(j+1))-real(A(j))); 
