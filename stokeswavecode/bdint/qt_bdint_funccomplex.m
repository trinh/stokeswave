function [F, theta] = qt_bdint_funccomplex(w, Y, solseries, number_of_pass, a)
...
G = 0*w;
for k = 1:length(w)    
    G(k) = qt_getint_gauss(w(k), solseries);    
end

% We set the 'bottom' sheet, i.e. does not require recursion
theta(number_of_pass,:) = 1/(1i*(-1)^(number_of_pass+1))*(G - tau(number_of_pass,:));
...
for j = number_of_pass-1:-1:1    
    ...
    theta(j,:) = a/(1i*(-1)^(j+1))*(G - tau(j,:) + summ);    
