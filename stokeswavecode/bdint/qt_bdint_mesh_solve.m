function pathdat = qt_bdint_mesh_solve(s, solseries, varargin)
    ...
    myparse.addRequired('solseries', @(x) true);
    ...
    myparse.parse(s, solseries, varargin{:})
    ...
    % If no initial value was specified, use the solseries
    ...
    for j = 1:length(s)-1
        ...
        elseif z1 == 999
            ...
            % First solve across
            fwd = @(s,U)qt_bdint_funccomplex(s + 1i*psi0, U, solseries, sheetnumber, a);
            ...
            for kk = 1:length(ss)
                ...
                tmp = taurow(kk,:);
                tmp = tmp(:);
                [~, thetaval] = qt_bdint_funccomplex(ss(kk) + 1i*psi0, tmp, solseries, sheetnumber, a);
            ...
            for k = 1:length(phi)
                ...
                fwd = @(s,U) 1i*qt_bdint_funccomplex(phi(k) + 1i*s, U, solseries, sheetnumber, a);
                ...
                for kk = 1:length(ss)
                    ...
                    tmp = uu(kk,:);
                    tmp = tmp(:);
                    [~, thetaval] = qt_bdint_funccomplex(phi(k) + 1i*ss(kk), tmp, solseries, sheetnumber, a);
                    tt(kk,:) = thetaval;
                end
