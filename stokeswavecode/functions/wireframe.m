...
function L=wireframe(X,Y,Z,N,w,col)
    ...
    nx1=Nx([1:N(1):end end],:);  nx1(:,end+1)=NaN;
    ny1=Ny([1:N(1):end end],:);  ny1(:,end+1)=NaN;
    nz1=Nz([1:N(1):end end],:);  nz1(:,end+1)=NaN;
    
    nx2=Nx(:,[1:N(2):end end]);  nx2(end+1,:)=NaN;
    ny2=Ny(:,[1:N(2):end end]);  ny2(end+1,:)=NaN;
    nz2=Nz(:,[1:N(2):end end]);  nz2(end+1,:)=NaN;
    
    
    set(gcf,'NextPlot','Add');                    % Add surf lines to current plot...
       
    L(1)=line(lin(XS1'),lin(YS1'),lin(ZS1'));     % Plot the latitudinal lines
    set(L(1),'Color',col, 'LineWidth', w);
    
    L(2)=line(XS2(:),YS2(:),ZS2(:));              % Plot the longitudinal lines
    set(L(2),'Color',col, 'LineWidth', w);
    
    % Modification to allow for line lighting
    %     L(1)=patch(lin(XS1'),lin(YS1'),lin(ZS1'), 'k');     % Plot the latitudinal lines
    %     set(L(1), 'LineWidth', w, 'EdgeColor', col, 'FaceColor', 'none', 'FaceLighting', 'none', 'EdgeLighting', 'phong', 'VertexNormals', [lin(nx1') lin(ny1') lin(nz1')]);
    %     L(2)=patch(XS2(:),YS2(:),ZS2(:), 'k');     % Plot the latitudinal lines
    %     set(L(2), 'LineWidth', w, 'EdgeColor', col, 'FaceColor', 'none', 'FaceLighting', 'none', 'EdgeLighting', 'phong', 'VertexNormals', [nx2(:) ny2(:) nz2(:)]);
    ...
    if nargin==0
        S=surf(X,Y,Z);
        set(S,'Edgecolor','none','BackfaceLighting','Lit', 'FaceLighting', 'phong', 'EdgeLighting', 'phong');
        view(45,45);
        l=light;
        axis equal
        camzoom(1.5);
