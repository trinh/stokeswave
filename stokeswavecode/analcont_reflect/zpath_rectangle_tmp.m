...
F = load('../../stokeswavedata/meshdat_M100_ep0.3.mat');
meshdat = F.meshdat;
...
recnum = 3; % No. of rectangles to plot
...
xlabel('phi'); ylabel('psi'); zlabel('imag(zp)');
legend(h, lcell)
...
for j = 2:length(recdat)
    h(j) = plot3(real(recdat(j).f), imag(recdat(j).f), imag(recdat(j).g), 'LineWidth', 3);
    lcell{j} = ['j = ', num2str(j)];
end

xlabel('phi'); ylabel('psi'); zlabel('imag(g)');
legend(h, lcell)
grid on
